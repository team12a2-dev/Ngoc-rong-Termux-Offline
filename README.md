...
| Item custom | Chọn loại quái từ catalog `mob_template`, chọn khoảng level người chơi rồi thêm item với tỷ lệ độc lập, quantity min–max và options dạng `option_id:param`. `Mob ID -1` áp dụng cho mọi quái; cùng một item có thể có rule riêng cho nhiều Mob ID và khoảng level. |
