        ...
        <main>
          <form className="control-card section" onSubmit={save}>
            ...
            <div className="drop-section-card">
              ...
              {catalog.length > 0 && <div className="table-wrap drop-catalog-quick"><table className="compact"><thead><tr><th>Icon</th><th>ID</th><th>Tên</th><th /><th /></tr></thead><tbody>{catalog.map((item) => <tr key={item.id}><td><ItemIcon iconId={item.iconId ?? item.icon_id} tempId={item.id} name={item.name} size={36} /></td><td><code>#{item.id}</code></td><td><strong>{item.name}</strong></td><td>{item.gender ?? '-'}</td><td><button className="btn sm" type="button" onClick={() => addItem(item)}>Thêm</button></td></tr>)}</tbody></table></div>}
              <div className="table-wrap drop-quick-items-table"><table className="compact"><thead><tr><th>Bật</th><th>Icon</th><th>Item</th><th>Tỷ lệ %</th><th>Số lượng từ</th><th>Số lượng đến</th><th /></tr></thead><tbody>
                {form.items.map((item, index) => <tr key={`${item.tempId}-${index}`}><td><input type="checkbox" checked={item.enabled} onChange={(e) => patchItem(index, 'enabled', e.target.checked)} /></td><td><ItemIcon iconId={item.iconId} tempId={item.tempId} name={itemLabel(item)} size={36} /></td><td><strong>{itemLabel(item)}</strong><small>#{item.tempId}</small></td><td><input className="input-xs" type="number" min="0" max="100" step="0.01" value={item.chancePercent} onChange={(e) => patchItem(index, 'chancePercent', numberValue(e.target.value))} /></td><td><input className="input-xs" type="number" min="1" value={item.quantityMin} onChange={(e) => patchItem(index, 'quantityMin', numberValue(e.target.value, 1))} /></td><td><input className="input-xs" type="number" min="1" value={item.quantityMax} onChange={(e) => patchItem(index, 'quantityMax', numberValue(e.target.value, 1))} /></td><td><button className="btn sm danger" type="button" onClick={() => removeItem(index)}>Xóa</button></td></tr>)}
                {form.items.length === 0 && <tr><td colSpan={7} className="muted">Chưa có item. Tìm item ở trên rồi bấm Thêm.</td></tr>}
              </tbody></table></div>
            </div>
