    ...
    <div>
      ...
      <div className="events-layout god-spin-layout">
        ...
        <form className="card section event-editor" onSubmit={save}>
          ...
          <div className="editor-tabs"><button type="button" className={`tab ${tab === 'pool' ? 'active' : ''}`} onClick={() => setTab('pool')}>Pool item ({form.items.length})</button><button type="button" className={`tab ${tab === 'general' ? 'active' : ''}`} onClick={() => setTab('general')}>Giá, lịch & giới hạn</button><button type="button" className={`tab ${tab === 'history' ? 'active' : ''}`} onClick={() => setTab('history')}>Lịch sử & tỷ lệ thực tế</button></div>
          {tab === 'general' && <>
          ...
          {tab === 'history' && <div className="spin-history-panel">
            <div className="section-title"><div><h3>Lịch sử quay thưởng</h3><p className="card-hint">Dữ liệu đọc trực tiếp từ `panel_god_spin_logs`; tỷ lệ thực tế = số lần trúng item / tổng lượt quay đã ghi nhận.</p></div><button type="button" className="btn sm" onClick={() => loadHistory(1)} disabled={historyLoading}>{historyLoading ? 'Đang tải...' : 'Làm mới'}</button></div>
            <div className="form-grid compact-form"><label className="field">Cấu hình vòng quay<select value={historyFilters.configId} onChange={(e) => setHistoryFilters((current) => ({ ...current, configId: e.target.value }))}><option value="">Tất cả cấu hình</option>{configs.map((config) => <option key={config.id} value={config.id}>{config.name} · #{config.id}</option>)}</select></label><label className="field">Người chơi / ID<input value={historyFilters.player} onChange={(e) => setHistoryFilters((current) => ({ ...current, player: e.target.value }))} placeholder="Nhập tên hoặc player ID" /></label><label className="field">Từ thời gian<input type="datetime-local" value={historyFilters.from} onChange={(e) => setHistoryFilters((current) => ({ ...current, from: e.target.value }))} /></label><label className="field">Đến thời gian<input type="datetime-local" value={historyFilters.to} onChange={(e) => setHistoryFilters((current) => ({ ...current, to: e.target.value }))} /></label></div>
            <div className="button-row"><button type="button" className="btn primary" onClick={() => loadHistory(1)} disabled={historyLoading}>Áp dụng bộ lọc</button><button type="button" className="btn" onClick={() => { const blankFilters = { configId: '', player: '', from: '', to: '', page: 1, pageSize: 25 }; setHistoryFilters(blankFilters); loadHistory(1, blankFilters); }}>Xóa bộ lọc</button></div>
            <div className="spin-summary"><div><strong>{history.summary.totalSpins}</strong><span>tổng lượt quay</span></div><div><strong>{history.summary.playerCount}</strong><span>người chơi</span></div><div><strong>{history.summary.configCount}</strong><span>cấu hình có log</span></div><div><strong>{history.pagination.total}</strong><span>dòng lịch sử</span></div></div>
            <div className="table-wrap"><table className="compact"><thead><tr><th>Item trúng</th><th>ID</th><th>Số lần trúng</th><th>Tỷ lệ thực tế</th></tr></thead><tbody>{history.distribution.map((row) => <tr key={row.tempId}><td><strong>{row.itemName}</strong></td><td>#{row.tempId}</td><td>{row.wins}</td><td><strong>{row.actualPercent.toFixed(4)}%</strong></td></tr>)}{history.distribution.length === 0 && <tr><td colSpan="4" className="muted">Chưa có dữ liệu trúng thưởng phù hợp.</td></tr>}</tbody></table></div>
            <div className="table-wrap"><table className="compact"><thead><tr><th>Thời gian</th><th>Người chơi</th><th>Cấu hình</th><th>Item trúng</th><th>SL</th></tr></thead><tbody>{history.history.map((row) => <tr key={row.id}><td>{row.createdAt ? new Date(row.createdAt).toLocaleString() : '—'}</td><td><strong>{row.playerName}</strong><small>#{row.playerId ?? '—'}</small></td><td>{row.configName || `#${row.configId ?? '—'}`}</td><td>{row.itemName || `Item #${row.tempId ?? '—'}`}</td><td>{row.spinCount}</td></tr>)}{history.history.length === 0 && <tr><td colSpan="5" className="muted">Chưa có lịch sử quay thưởng.</td></tr>}</tbody></table></div>
            <div className="button-row"><button type="button" className="btn sm" onClick={() => loadHistory(Math.max(1, history.pagination.page - 1))} disabled={historyLoading || history.pagination.page <= 1}>← Trang trước</button><span className="muted">Trang {history.pagination.page}/{history.pagination.totalPages} · {history.pagination.total} dòng</span><button type="button" className="btn sm" onClick={() => loadHistory(Math.min(history.pagination.totalPages, history.pagination.page + 1))} disabled={historyLoading || history.pagination.page >= history.pagination.totalPages}>Trang sau →</button></div>
          </div>}
          {tab !== 'history' && <div className="form-actions"><button className="btn primary" type="submit" disabled={loading}>{loading ? 'Đang lưu...' : 'Lưu SQL & reload runtime'}</button><span className="muted">Các option được lưu trong `options_json`; SQL vẫn là nguồn sự thật sau restart.</span></div>}
        </form>
      </div>
    </div>
  );
