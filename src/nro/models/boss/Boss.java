...
import nro.models.boss.spawn.BossSpawnOrchestrator;
...
public class Boss extends Player implements IBoss {
    ...
    /** Thời gian nghỉ thực tế (ms) sau jitter / lịch spawn */
    protected long nextRestDelayMs;
    public Boss getParentBoss() {
        return parentBoss;
    }

    public void setParentBoss(Boss parent) {
    ...
    public void setLastTimeRest(long timeMs) {
        this.lastTimeRest = timeMs;
    }
...
public Player bomAttacker; // thêm dòng này
    ...
    public Zone getMapJoin() {
        ...
        if (BossSpawnConfig.distributionEnabled && this.data[nextLevel].getMapJoin().length > 1) {
            mapId = BossSpawnOrchestrator.pickMapId(this, nextLevel);
        } else {
            mapId = this.data[nextLevel].getMapJoin()[Util.nextInt(0, this.data[nextLevel].getMapJoin().length - 1)];
        }
...
if (prepareBom && Util.canDoWithTime(lastBomTime, 2500)) {
    ...
    /** Đặt thời điểm REST và random cooldown spawn kế tiếp */
    protected void markRestAndSchedule() {
        this.lastTimeRest = System.currentTimeMillis();
        BossSpawnSchedule.onEnterRest(this);
    }

    @Override
    public void rest() {
        if (BossSpawnSchedule.canSpawnFromRest(this, BossSpawnSchedule.resolveRestSpawnLevel(this))) {
