...
public class DataGame {
    ...
    public static void updateData(MySession session) {
        ...
        try {
            ...
            Logger.warning("updateData: gui du lieu cho " + session.ipAddress + "\n");
            if (session.getNumMessages() == 0) {
    ...
    public static void sendDataImageVersion(MySession session) {
        Message msg = null;
        try {
            final byte[] data = FileIO.readFile("data/update_data/image");
            if (data == null || data.length == 0) {
                Logger.warning("sendDataImageVersion: thieu file data/update_data/image\n");
                return;
            }
            msg = new Message(-111);
    ...
    public static void sendIcon(MySession session, int id) {
        ...
        sendIconBytes(session, id, icon);
    }

	public static boolean sendIconBytes(MySession session, int id, byte[] icon) {
        if (session == null || icon == null || icon.length == 0) {
            return false;
        }
        Message msg = null;
        try {
...
//            }
    ...
    public static void sendImageByName(MySession session, String imgName) {
        ...
        try {
            ...
            final byte[] data = FileIO.readFile("data/img_by_name/x" + resolveZoomLevel(session) + "/" + imgName + ".png");
    ...
    public static void sendVersionRes(ISession session) {
        ...
        try {
            ...
            session.sendMessage(msg);
            msg.cleanup();
            if (session instanceof MySession mySession) {
                Logger.warning("sendVersionRes: vsRes=" + vsRes + " (" + mySession.ipAddress + ")\n");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static final ExecutorService RES_SENDER = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "DataGame-sendRes");
        t.setDaemon(true);
        return t;
    });
    ...
    public static void sendSizeRes(MySession session) {
        Message msg;
        try {
            byte zoom = resolveZoomLevel(session);
            ...
            session.sendMessage(msg);
            msg.cleanup();
            Logger.warning("sendSizeRes: x" + zoom + " -> " + count + " file (" + session.ipAddress + ")\n");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void sendRes(MySession session) {
        ...
        RES_SENDER.execute(() -> sendResSync(session));
    }

    private static final int RES_SLEEP_MS = 8;
    private static final int RES_QUEUE_PAUSE = 25;

    private static void sendResSync(MySession session) {
        Message msg = null;
        try {
            byte zoom = resolveZoomLevel(session);
            session.zoomLevel = zoom;
            ...
            Logger.warning("sendRes: bat dau gui " + total + " file x" + zoom + " (" + session.ipAddress + ")\n");
            if (files != null) {
                int sent = 0;
                for (final File fileEntry : files) {
                    if (!session.isConnected()) {
                        Logger.warning("sendRes: client ngat ket noi sau " + sent + "/" + total + " file\n");
                        return;
                    }
                    while (session.getNumMessages() > RES_QUEUE_PAUSE) {
                        Thread.sleep(RES_SLEEP_MS);
                        if (!session.isConnected()) {
                            return;
                        }
                    }
                    ...
                    try (FileChannel fileChannel = FileChannel.open(fileEntry.toPath(), StandardOpenOption.READ)) {
                        ...
                        sent++;
                        if (sent % 100 == 0) {
                            Logger.warning("sendRes: " + sent + "/" + total + " (" + session.ipAddress + ")\n");
                        }
                        Thread.sleep(RES_SLEEP_MS);
                    } catch (IOException e) {
                        Logger.logException(DataGame.class, e);
            ...
            session.sendMessage(msg);
            msg.cleanup();
            Logger.warning("sendRes: hoan tat x" + zoom + " (" + session.ipAddress + ")\n");
        } catch (Exception e) {
            if (msg != null) {
                msg.cleanup();
            }
    ...
    public static void handleDownloadPacket(MySession session, Message msg) {
        ...
        try {
            ...
            if (avail >= 41) {
                ...
                if (ClientVerifier.ENABLED && !ClientVerifier.verify(token)) {
                    Logger.warning("[BLOCK] " + session.ipAddress + " token không hợp lệ → kick\n");
                    session.disconnect();
                    return;
                }
            ...
            Logger.warning("download type=" + type + " bytes=" + avail + " ip=" + session.ipAddress + "\n");
            if (type == 0) {
                sendVersionRes(session);
            } else if (type == 1) {
                sendSizeRes(session);
            } else if (type == 2) {
                sendRes(session);
    ...
    private static byte resolveZoomLevel(MySession session) {
        byte zoom = session == null ? 2 : session.zoomLevel;
        if (zoom >= 1 && zoom <= 4 && new File("data/res/x" + zoom).isDirectory()) {
            return zoom;
        }
        for (byte candidate : new byte[]{2, 4, 3, 1}) {
            if (new File("data/res/x" + candidate).isDirectory()) {
                return candidate;
            }
        }
        return 2;
