...
public class TopBanDoKhoBau {
    ...
    public void load() {
        ...
        try (
            ...
            PreparedStatement ps = con.prepareStatement("SELECT *, " + "SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichBang, \',\', 1), \'[\', -1) AS so1, " + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichBang, \',\', 2), \',\', -1) AS SIGNED) AS so2, " + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichBang, \',\', 3), \',\', -1) AS SIGNED) AS so3, " + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichBang, \',\', 4), \',\', -1) AS SIGNED) AS so4 " + "FROM player " + "WHERE CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichBang, \',\', 2), \',\', -1), \']\', 1) AS SIGNED) > 0 " + "ORDER BY so2 DESC, so1 ASC LIMIT 100")) {
