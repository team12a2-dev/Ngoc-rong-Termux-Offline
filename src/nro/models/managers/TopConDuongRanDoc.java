...
public class TopConDuongRanDoc {
    ...
    public void load() {
        ...
        try (
            ...
            PreparedStatement ps = con.prepareStatement("SELECT *, " + "SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichCDRD, \',\', 1), \'[\', -1) AS so1, " + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichCDRD, \',\', 2), \',\', -1) AS SIGNED) AS so2, " + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichCDRD, \',\', 3), \',\', -1) AS SIGNED) AS so3, " + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichCDRD, \',\', 4), \',\', -1) AS SIGNED) AS so4 " + "FROM player " + "WHERE CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichCDRD, \',\', 2), \',\', -1), \']\', 1) AS SIGNED) > 0 " + "ORDER BY so2 DESC, so1 ASC LIMIT 100")) {
