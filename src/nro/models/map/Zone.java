...
public class Zone {
                        ...
                        if (playerId == player.id || itemMap.playerId == player.id || itemMap.playerId == -1) {
                            ...
                            if (InventoryService.gI().addItemBag(player, item)) {
                                ...
                                try {
                                    ...
                                    if (!(this.map.mapId >= 21 && this.map.mapId <= 23 && itemMap.itemTemplate != null && itemMap.itemTemplate.id == 74 || this.map.mapId >= 42 && this.map.mapId <= 44 && itemMap.itemTemplate != null && itemMap.itemTemplate.id == 78)) {
        ...
        try {
            if (player.zone != null) {
                ...
                if (MapService.gI().isMapOffline(this.map.mapId)) {
                    ...
                    for (int i = players.size() - 1; i >= 0; i--) {
                        Player pl = players.get(i);
                        if (!player.equals(pl) && shouldSendBossToPlayer(player, pl)) {
                            infoPlayer(pl, player);
                        }
                    }
                }
    ...
    public void load_Another_To_Me(Player player) {
        try {
            ...
            if (MapService.gI().isMapOffline(this.map.mapId)) {
                ...
                for (int i = this.humanoids.size() - 1; i >= 0; i--) {
                    Player pl = this.humanoids.get(i);
                    if (pl != null && !player.equals(pl) && shouldSendBossToPlayer(player, pl)) {
                        infoPlayer(player, pl);
                    }
                }
            }
    ...
    private boolean shouldSendBossToPlayer(Player viewer, Player entity) {
        if (entity == null || !entity.isBoss) {
            return true;
        }
        if (entity instanceof TrainingBoss trainingBoss) {
            return trainingBoss.playerAtt != null && trainingBoss.playerAtt.equals(viewer);
        }
        return true;
    }
    ...
    public void mapInfo(Player pl) {
        ...
        try {
            ...
            pl.sendMessage(msg);
            msg.cleanup();
            // Đảm bảo luôn gỡ màn hình "Xin chờ" (Mabư 12h) khi vào bất kỳ map nào
    ...
    public boolean isKhongCoTrongTaiTrongKhu() {
        ...
        for (Player pl : players) {
            ...
            if ((pl.zone.map.mapId >= 21 && pl.zone.map.mapId <= 23) || pl.zone.map.mapId == 170 || pl.zone.map.mapId == 153 || pl.zone.map.mapId == 52 || pl.zone.map.mapId == 113 || pl.zone.map.mapId == 129 || MapService.gI().isMapDoanhTrai(pl.zone.map.mapId) || MapService.gI().isMapBlackBallWar(pl.zone.map.mapId) || MapService.gI().isMapBanDoKhoBau(pl.zone.map.mapId) || MapService.gI().isMapPhoBan(pl.zone.map.mapId) || MapService.gI().isMapMaBu(pl.zone.map.mapId) || MapService.gI().isMapKhiGasHuyDiet(pl.zone.map.mapId) || MapService.gI().isMapConDuongRanDoc(pl.zone.map.mapId) || MapService.gI().isMapOffline(pl.zone.map.mapId)) {
                kovao = false;
            }
        }
        return kovao;
    ...
    @java.lang.SuppressWarnings("all")
    public void setNpc(final Player Npc) {
        this.Npc = Npc;
    }
