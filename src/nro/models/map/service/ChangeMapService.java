...
public class ChangeMapService {
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (MapService.gI().isMapKhiGasHuyDiet(pl.zone.map.mapId)) {
            Service.gI().sendThongBao(pl, "Không thể đổi khu vực trong map Khí Gas Hủy Diệt");
            return;
        }
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null && pl.zone != null) {
            ...
            if (zoneJoin.map.mapId == ConstMap.DONG_NAM_KARIN
                    && !MapService.gI().canEnterDongNamKarin(pl.nPoint.power)) {
                ...
                Service.gI().sendThongBao(pl, MapService.MSG_DONG_NAM_KARIN_POWER_LIMIT);
                return;
            }
            ...
            if (pl.isPl() && nextMapIsMabu) {
                Service.gI().changeFlag(pl, 9);
                MajinBuuService.gI().ensureBossesCombatReady(zoneJoin);
    ...
    public void finishLoadMap(Player player) {
        ...
        if (player.mobMe != null) {
            player.mobMe.spawnToOwnerAfterLoadMap();
        }
    }
