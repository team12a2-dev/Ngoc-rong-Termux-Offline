...
import nro.models.managers.TopVoDaiSinhTu;
import static nro.models.utils.Util.setTimeout;

public class DeathOrAliveArenaService {

    /** Sức mạnh tối đa được tham gia võ đài sinh tử */
    public static final long POWER_LIMIT_VDST = 150_000_000L;
    public static final String MSG_POWER_LIMIT = "Sức mạnh của bạn trên 150 triệu, không thể tham gia võ đài sinh tử.";
    ...
    public boolean canJoinVoDaiSinhTu(Player player) {
        return player != null && player.nPoint != null && player.nPoint.power <= POWER_LIMIT_VDST;
    }

    public void showTop100(Player player) {
        TopVoDaiSinhTu.getInstance().show(player);
    }

    public void startChallenge(Player player) {
        if (!canJoinVoDaiSinhTu(player)) {
            Service.gI().sendThongBao(player, MSG_POWER_LIMIT);
            return;
        }
