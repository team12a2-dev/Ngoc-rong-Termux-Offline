...
public final class WorldMartialArtsTournament implements Runnable {
  ...
  if (plWin != null
    && WorldMartialArtsTournamentManager.gI().round == 2
    && plWin.playerTask != null
    && plWin.playerTask.taskMain != null
    && plWin.playerTask.taskMain.id == 17
    && plWin.playerTask.taskMain.index == 3) {

    TaskService.gI().checkDoneTaskWinAnyOpponent(plWin);

    Service.gI().sendThongBao(
        plWin,
        "Bạn đã hoàn thành nhiệm vụ: Thắng vòng 2 Đại Hội Võ Thuật"
    );
}
