...
public class ConSoMayManGold implements Runnable {
    ...
    public static final long DEFAULT_BET = 1;
    public static final long MIN_CUSTOM_TIER = 5;
    public static final long MAX_CUSTOM_TIER = 500;
    ...
    public boolean isCustomTier(Player player) {
        return player != null && player.csmmBetTierGold > 0;
    }

    public long getBetCost(Player player) {
        if (isCustomTier(player)) {
            return player.csmmBetTierGold;
        }
        return DEFAULT_BET;
    }
    ...
    public boolean setPlayerBetTier(Player player, long tier) {
        if (tier < MIN_CUSTOM_TIER || tier > MAX_CUSTOM_TIER) {
            Service.gI().sendThongBao(player, "Mức cược phải từ " + MIN_CUSTOM_TIER + " đến " + MAX_CUSTOM_TIER + " thỏi vàng");
            return false;
        }
        player.csmmBetTierGold = tier;
        long totalWin = calcTotalWin(tier);
        Service.gI().sendThongBao(player, "Chế độ tùy chọn: cược " + tier + " thỏi vàng, thắng nhận "
                + totalWin + " thỏi (= giải " + rewardAmount + " + cược " + tier + ").");
        return true;
    }

    public void useDefaultBetTier(Player player) {
        player.csmmBetTierGold = 0;
        long totalWin = calcTotalWin(DEFAULT_BET);
        Service.gI().sendThongBao(player, "Đã về mức cược mặc định " + DEFAULT_BET + " thỏi vàng. Thắng nhận "
                + totalWin + " thỏi (= giải " + rewardAmount + " + cược " + DEFAULT_BET + ").");
    }
    ...
    private boolean placeBet(Player player, int point, long betCost) {
        ...
        if (betCost < DEFAULT_BET || betCost > MAX_CUSTOM_TIER) {
            Service.gI().sendThongBao(player, "Mức cược phải từ " + DEFAULT_BET + " đến " + MAX_CUSTOM_TIER + " thỏi vàng");
            return false;
        }
        ...
        synchronized (lock) {
            ...
            if (thoiVang == null || thoiVang.quantity < betCost) {
                Service.gI().sendThongBao(player, "Bạn không đủ " + betCost + " thỏi vàng để thực hiện");
                return false;
            }
            ...
            players.add(data);
            InventoryService.gI().subQuantityItemsBag(player, thoiVang, (int) betCost);
            InventoryService.gI().sendItemBags(player);
        }

        Service.gI().sendThongBao(player, "Bạn đã chọn số " + point + " | Cược " + betCost + " thỏi vàng | Thắng nhận "
                + winAmount + " thỏi (= giải " + roundPrize + " + cược " + betCost + ").");
    ...
    private void payWinner(Player player, ConSoMayManData g) {
        if (player == null || g.conSoMayManVang != 1) {
            return;
        }
        long winAmount = g.rewardAmount;
        Item item = ItemService.gI().createNewItem((short) 457, (int) winAmount);
    ...
    private String buildFinishMessage(int id, ConSoMayManData g) {
        ...
        if (currentPlayer != null) {
            return "Chúc mừng " + currentPlayer.name
                    + " đã thắng " + Util.numberToMoney(winAmount)
                    + " thỏi vàng với con số may mắn " + result;
        }
        return "Chúc mừng người chơi ID: " + g.id + " đã thắng " + Util.numberToMoney(winAmount) + " thỏi vàng với con số may mắn " + result;
    }
    ...
    private void announceWorldWinner(ConSoMayManData g) {
        ...
        Service.gI().sendThongBaoAllPlayer("Chúc mừng " + name + " trúng Con số may mắn thỏi vàng số "
                + result + ", nhận " + Util.numberToMoney(g.rewardAmount) + " thỏi vàng!");
    }
