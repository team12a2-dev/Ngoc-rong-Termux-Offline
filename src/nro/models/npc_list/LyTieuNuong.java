...
public class LyTieuNuong extends Npc {
    ...
    public void confirmMenu(Player player, int select) {
        ...
        if (canOpenNpc(player)) {
            if (this.mapId == 5) {
                    ...
                    switch (select) {
                        case 0: // Kéo, Búa, Bao
                            createOtherMenu(player, ConstNpc.KEO_BUA_BAO, "Hãy chọn mức cược.",
                                    ...
                                    "10 Tỷ vàng");
                            break;
                        case 1: // Con số may mắn
                            createOtherMenu(player, ConstNpc.CON_SO_MAY_MAN, "Con số may mắn",
                                    "Con số\nmay mắn\nthỏi vàng",
                                    "Con số\nmay mắn\nngọc xanh");
                            break;
                        case 2: // Chọn ai đây
                ...
                } else if (player.idMark.getIndexMenu() == ConstNpc.CON_SO_MAY_MAN) {
                    switch (select) {
                        case 0 ->
    ...
    public void refreshConSoMayManGemMenu(Player player) {
        xửLýLựaChọnMiniGame_Gem(player);
    }

    public void refreshConSoMayManGoldMenu(Player player) {
        xửLýLựaChọnMiniGame_Gold(player);
    }
    ...
    private void xửLýLựaChọnMiniGame_Gem(Player player) {
        ...
        String KQ = String.valueOf(game.result);
        String Money = String.valueOf(game.rewardAmount);
        String timerLine = game.getTimerDisplayLine();
        String number = game.strNumberDetail((int) player.id);
        if (number == null || number.isEmpty()) {
            number = game.strNumber((int) player.id);
        }

        StringBuilder previousResults = new StringBuilder("Lịch sử: ");
        if (game.dataKQ_CSMM != null && !game.dataKQ_CSMM.isEmpty()) {
            int maxResultsToShow = Math.min(10, game.dataKQ_CSMM.size());
            for (int i = game.dataKQ_CSMM.size() - maxResultsToShow; i < game.dataKQ_CSMM.size(); i++) {
                previousResults.append(game.dataKQ_CSMM.get(i));
                if (i < game.dataKQ_CSMM.size() - 1) {
        ...
        String npcSay = ""
                ...
                + "Giải thưởng vòng này: " + Money + " ngọc xanh\n"
                + "Mức cược mặc định: " + defaultBet + " ngọc xanh\n"
                + "Thắng nhận: " + defaultWin + " ngọc\n"
                + timerLine + "\n"
                + (number != null && !number.isEmpty() ? "Các số bạn chọn: " + number : "Bạn chưa chọn số nào.");
        String[] Menus = {
                "Cập nhật",
                "1 Số\n" + defaultBet + " ngọc xanh",
                "Ngẫu nhiên\n1 số lẻ\n" + defaultBet + " ngọc xanh",
                "Ngẫu nhiên\n1 số chẵn\n" + defaultBet + " ngọc xanh",
                "Chọn mức\ncược",
                "Đóng"
    ...
    private void xửLýLựaChọnMiniGame_Gold(Player player) {
        ...
        String KQ = String.valueOf(game.result);
        String Money = Util.mumberToBlue(game.rewardAmount);
        String timerLine = game.getTimerDisplayLine();
        String number = game.strNumberDetail((int) player.id);
        if (number == null || number.isEmpty()) {
            number = game.strNumber((int) player.id);
        ...
        String npcSay = ""
                + "Kết quả giải trước: " + KQ + "\n"
                + previousResults.toString() + "\n"
                + "Giải thưởng vòng này: " + Money + " thỏi vàng\n"
                + "Mức cược mặc định: " + defaultBet + " thỏi vàng\n"
                + "Thắng nhận: " + defaultWin + " thỏi\n"
                + timerLine + "\n"
                + (number != null && !number.isEmpty() ? "Các số bạn chọn: " + number : "Bạn chưa chọn số nào.");
        String[] Menus = {
                "Cập nhật",
                "1 Số\n" + defaultBet + " thỏi vàng",
                "Ngẫu nhiên\n1 số lẻ\n" + defaultBet + " thỏi vàng",
                "Ngẫu nhiên\n1 số chẵn\n" + defaultBet + " thỏi vàng",
                "Chọn mức\ncược",
                "Đóng"
    ...
    private void xửLýConSoMayManGem(Player player, int select) {
        switch (select) {
            ...
            case 3:
                ...
                break;
            case 4:
                Input.gI().createFormConSoMayManGemTuyChon(player);
                break;
        }
    }
    ...
    private void xửLýConSoMayManGold(Player player, int select) {
        switch (select) {
            ...
            case 3:
                ...
                break;
            case 4:
                Input.gI().createFormConSoMayManGoldTuyChon(player);
                break;
        }
    }
