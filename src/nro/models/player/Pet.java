...
public class Pet extends Player {
    ...
    public void fusionGogeta(boolean porata) {
        ...
        if (Util.canDoWithTime(lastTimeUnfusion, TIME_WAIT_AFTER_UNFUSION)) {
            ...
            Service.gI().sendThongBao(this.master, "Vui lòng đợi " + TimeUtil.getTimeLeft(lastTimeUnfusion, TIME_WAIT_AFTER_UNFUSION / 1000) + " nữa");
        }
    }
    ...
    private void moveIdle() {
        ...
        if (idle && Util.canDoWithTime(lastTimeMoveIdle, timeMoveIdle)) {
            ...
            PlayerService.gI().playerMove(this, master.location.x + (dir == -1 ? 50 : -50), master.location.y);
            ...
            if (this.master != null && this.master.zone != null) {
                ...
                case PROTECT: 
                    ...
                    if (playerAttack != null) {
                        if ((this.typePet == 2 || this.typePet == 4 || this.typePet == 5) && Util.isTrue(1, 5) && playerAttack.nPoint.hp < 1000000000 && !playerAttack.nPoint.islinhthuydanhbac && !playerAttack.isBoss) {
                    ...
                    if (this.zone != null && (this.zone.map.mapId == 21 || this.zone.map.mapId == 22 || this.zone.map.mapId == 23)) {
                        if (System.currentTimeMillis() - lastTimeMoveAtHome > 5000) {
                            int[] coords = switch (this.zone.map.mapId) {
                                case 21 -> directAtHome == -1 ? new int[] {250, 336} : new int[] {200, 336};
                                case 22 -> directAtHome == -1 ? new int[] {500, 336} : new int[] {452, 336};
    ...
    private boolean useSkill5() {
        try {
            ...
            boolean canUse = SkillService.gI().canUseSkillWithCooldown(this) && SkillService.gI().canUseSkillWithMana(this);
    ...
    private boolean cantAttack(Player player) {
        return player == null || player.location == null || player.isDie() || Util.getDistance(this, player) > 500 || this.equals(player) || (player.equals(master) || this.typePet != 5 && this.typePet != 2 && this.typePet != 4) || (!temporaryEnemies.contains(player) && !master.temporaryEnemies.contains(player)) || (!SkillService.gI().canAttackPlayer(this, player));
    }
    ...
    public void openSkill5() {
        Skill skill = null;
        switch (this.gender) {
            case 0 -> skill = SkillUtil.createSkill(Skill.SUPER_KAME, 1);
            case 1 -> skill = SkillUtil.createSkill(Skill.MA_PHONG_BA, 1);
            case 2 -> skill = SkillUtil.createSkill(Skill.LIEN_HOAN_CHUONG, 1);
            default -> {
                return;
            }
        }
    ...
    public void petSay(Player player) {
        if (this.typePet == 4) {
            if (Util.canDoWithTime(lastTimeChat, indexChat == 0 ? 15000 : 1500)) {
                String[] chat = {"Ta chính là thế giới", "Ta chính là công lí", "Hãy chiêm ngưỡng vẻ đẹp của ta! Hỡi con người", "Sức mạnh to lớn nằm trong cơ thể bất tử", "Ta sẽ đem công lí tới toàn bộ vũ trụ này"};
            ...
            if (Util.canDoWithTime(lastTimeChat, indexChat == 0 ? 15000 : 1500)) {
                String[] chat = {"Mày chán sống rồi à " + player.name + "?", "Mày muốn chết đúng không?", "Ngày này năm sau", "Tao sẽ nhớ uống thật nhiều nước", "Để đái vào mộ mày"};
                Service.gI().chat(this, chat[indexChat]);
                indexChat = (indexChat + 1) % chat.length;
                lastTimeChat = System.currentTimeMillis();
            }
