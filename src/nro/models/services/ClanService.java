    ...
    private void acceptAskJoinClan(Player player, int clanMessageId) {
        ...
        if (clan != null && clan.isLeader(player)) {
            ...
            if (cmg != null && !existInClan) {
                ...
                if (pl == null) {
                    pl = AmodsubVN.loadById(plxinvao);
                }
                if (pl != null) {
                    if (pl.idMark.isHoldBlackBall()) {
...
//                        }
    ...
    private static final long RECHECK_CLAN_COOLDOWN_MS = 60_000L;

    /**
     * Đồng bộ lại bang hội của người chơi từ database (dùng khi dữ liệu lệch).
     */
    public void recheckClan(Player player) {
        if (player == null) {
            return;
        }
        if (!Util.canDoWithTime(player.lastTimeRecheckClan, RECHECK_CLAN_COOLDOWN_MS)) {
            Service.gI().sendThongBao(player, "Vui lòng đợi " + TimeUtil.getTimeLeft(player.lastTimeRecheckClan, 60) + " nữa để kiểm tra lại bang hội.");
            return;
        }
        player.lastTimeRecheckClan = System.currentTimeMillis();
        int clanId = getClanIdFromDatabase((int) player.id);
        try {
            if (player.clan != null) {
                player.clan.removeMemberOnline(player.clanMember, player);
            }
            ...
            if (clanId == -1) {
                ...
                Service.gI().sendThongBao(player, "Kiểm tra xong: bạn hiện không có bang hội.");
                return;
            }
            ...
            ClanMember cm = clan.getClanMember((int) player.id);
            if (cm == null) {
                removeClanPlayer((int) player.id);
                player.clan = null;
                player.clanMember = null;
                sendMyClan(player);
                sendClanId(player);
                Service.gI().sendFlagBag(player);
                ItemTimeService.gI().removeTextDoanhTrai(player);
                Service.gI().sendThongBao(player, "Kiểm tra xong: không tìm thấy bạn trong bang, đã xóa liên kết.");
                return;
            }
            ...
            Service.gI().sendFlagBag(player);
            ItemTimeService.gI().sendTextDoanhTrai(player);
            Service.gI().sendThongBao(player, "Đã kiểm tra lại bang hội: " + clan.name);
        } catch (Exception e) {
            Logger.logException(ClanService.class, e, "recheckClan");
            Service.gI().sendThongBao(player, "Không thể kiểm tra bang hội, vui lòng thử lại sau.");
        }
    }

    private int getClanIdFromDatabase(int playerId) {
        try (Connection con = LocalManager.getConnection();
                PreparedStatement ps = con.prepareStatement("SELECT clan_id FROM player WHERE id = ?")) {
            ...
            Logger.logException(ClanService.class, e, "getClanIdFromDatabase");
        }
        return -1;
    }
