...
public class SkillService {
    ...
    public boolean useSkill(Player player, Player plTarget, Mob mobTarget, int status, Message msg) {
        ...
        if (plTarget == null && mobTarget == null && !player.isBoss) {
            plTarget = findNearestBossTarget(player);
        }
        ...
        if (player.playerSkill.skillSelect.template == null) {
            return false;
        }

        // Chặn skill tấn công (type 1) khi không có mục tiêu nào
        // để tránh kích hoạt cooldown "rỗng" rồi block đòn đánh thật
        int skillTempId = player.playerSkill.skillSelect.template.id;
        if (player.playerSkill.skillSelect.template.type == 1
                ...
                && skillTempId != Skill.KAIOKEN) {
            return false;
        }

        boolean canUseMana = canUseSkillWithMana(player);
        boolean canUseCooldown = canUseSkillWithCooldown(player);
        if (player.playerSkill.skillSelect.template.type == 2 && canUseMana && canUseCooldown) {
            useSkillBuffToPlayer(player, plTarget);
            return true;
        }
        boolean hasEffectSkill = player.effectSkill != null && player.effectSkill.isHaveEffectSkill();
        boolean canAttPl = plTarget == null || canAttackPlayer(player, plTarget);
    ...
    private void phanSatThuong(Player plAtt, Player plTarget, long dame) {
        ...
        if (plAtt instanceof TaoPaiPai tpp
                && (tpp.zone == null || tpp.zone.map == null || tpp.zone.map.mapId != ConstMap.DONG_NAM_KARIN)) {
            return;
        }
        ...
        int damePST = (int) Math.min(dame * (long) percentPST / 100L, 2_147_483_647L);
        if (damePST <= 0) {
            return;
        }
        Message msg = null;
    ...
    public boolean canAttackPlayer2(Player p1, Player p2) {
        ...
        if (p1.zone != null && p2.zone != null && MapService.gI().isMapMaBu(p1.zone.map.mapId)) {
            if (p1.isBoss && (p2.isPl() || p2.isPet)) {
                return true;
            }
            if (p2.isBoss && (p1.isPl() || p1.isPet)) {
                return true;
            }
            if (p1.isBoss && p2.isBoss
                    && p1.cFlag != 0 && p2.cFlag != 0 && p1.cFlag != p2.cFlag) {
                return true;
            }
        }

        if (p2.isBoss && (p1.isPl() || p1.isPet) && p1.zone != null && p2.zone != null
                && p1.zone.equals(p2.zone) && canHitBoss(p2)) {
            return true;
        }
        if (p1.isBoss && (p2.isPl() || p2.isPet) && p1.zone != null && p2.zone != null
                && p1.zone.equals(p2.zone) && canHitBoss(p1)) {
            return true;
        }
    ...
    private boolean canHitBoss(Player bossPlayer) {
        if (bossPlayer == null || bossPlayer.isDie()) {
            return false;
        }
        if (bossPlayer.typePk == ConstPlayer.PK_ALL) {
            return true;
        }
        if (bossPlayer instanceof Boss boss) {
            return boss.bossStatus == BossStatus.ACTIVE
    ...
    /** Tự tìm boss gần nhất khi client không gửi id mục tiêu (-60). */
    private Player findNearestBossTarget(Player player) {
        if (player == null || player.zone == null || player.playerSkill == null
                || player.playerSkill.skillSelect == null) {
            return null;
        }
        int range = SkillUtil.isUseSkillChuong(player)
                ? Skill.RANGE_ATTACK_CHIEU_CHUONG
                : Skill.RANGE_ATTACK_CHIEU_DAM;
        Player nearest = null;
        int minDist = range + 1;
        for (Player boss : player.zone.getBosses()) {
            if (boss == null || !boss.isBoss || boss.isDie() || !canHitBoss(boss)) {
                continue;
            }
            int dist = Util.getDistance(player, boss);
