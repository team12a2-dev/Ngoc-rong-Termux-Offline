...
public static final int ADMIN_ADD_VND = 600;
    ...
    public static final int CON_SO_MAY_MAN_GEM_MUC = 35;
    public static final int CON_SO_MAY_MAN_VANG_MUC = 36;
...
player.inventory.gem += soGem;
                ...
                case CON_SO_MAY_MAN_NGOC -> {
                    ...
                    ConSoMayManGem gameGem = MiniGame.gI().MiniGame_S1_Gem;
                    if (CSMM2 < gameGem.min || CSMM2 > gameGem.max) {
                        Service.gI().sendThongBao(player, "Số phải từ " + gameGem.min + " đến " + gameGem.max + ".");
                    } else if (gameGem.newData(player, CSMM2)) {
                        refreshConSoMayManGemMenu(player);
                    }
                }
                case CON_SO_MAY_MAN_VANG -> {
                    int CSMM2 = Integer.parseInt(text[0]);
                    ConSoMayManGold gameGold = MiniGame.gI().MiniGame_S1_Gold;
                    if (CSMM2 < gameGold.min || CSMM2 > gameGold.max) {
                        Service.gI().sendThongBao(player, "Số phải từ " + gameGold.min + " đến " + gameGold.max + ".");
                    } else if (gameGold.newData(player, CSMM2)) {
                        refreshConSoMayManGoldMenu(player);
                    }
                }
                case CON_SO_MAY_MAN_GEM_MUC -> {
                    int soChon = Integer.parseInt(text[0]);
                    long mucCuoc = Long.parseLong(text[1]);
                    ConSoMayManGem gameGem = MiniGame.gI().MiniGame_S1_Gem;
                    if (soChon < gameGem.min || soChon > gameGem.max) {
                        Service.gI().sendThongBao(player, "Số phải từ " + gameGem.min + " đến " + gameGem.max + ".");
                    } else if (!gameGem.isValidCustomBetAmount(mucCuoc)) {
                        Service.gI().sendThongBao(player, "Mức cược phải từ " + ConSoMayManGem.DEFAULT_BET + " đến " + ConSoMayManGem.MAX_CUSTOM_TIER + " ngọc xanh.");
                    } else if (gameGem.newDataWithBet(player, soChon, mucCuoc)) {
                        refreshConSoMayManGemMenu(player);
                    }
                }
                case CON_SO_MAY_MAN_VANG_MUC -> {
                    int soChon = Integer.parseInt(text[0]);
                    long mucCuoc = Long.parseLong(text[1]);
                    ConSoMayManGold gameGold = MiniGame.gI().MiniGame_S1_Gold;
                    if (soChon < gameGold.min || soChon > gameGold.max) {
                        Service.gI().sendThongBao(player, "Số phải từ " + gameGold.min + " đến " + gameGold.max + ".");
                    } else if (!gameGold.isValidCustomBetAmount(mucCuoc)) {
                        Service.gI().sendThongBao(player, "Mức cược phải từ " + ConSoMayManGold.DEFAULT_BET + " đến " + ConSoMayManGold.MAX_CUSTOM_TIER + " thỏi vàng.");
                    } else if (gameGold.newDataWithBet(player, soChon, mucCuoc)) {
                        refreshConSoMayManGoldMenu(player);
                    }
                }
                case GIVE_IT -> {
    ...
    public void createFormConSoMayMan_Gold(Player pl) {
        ConSoMayManGold game = MiniGame.gI().MiniGame_S1_Gold;
        long defaultBet = ConSoMayManGold.DEFAULT_BET;
        long defaultWin = game.calcTotalWin(defaultBet);
        createForm(pl, CON_SO_MAY_MAN_VANG,
                "Chọn số từ 0 đến 99\nCược: " + defaultBet + " thỏi vàng | Thắng: " + defaultWin
                        + " thỏi (= giải " + game.rewardAmount + " + cược " + defaultBet + ")",
                new SubInput("Số bạn chọn", NUMERIC));
    }

    public void createFormConSoMayMan_Gem(Player pl) {
        ConSoMayManGem game = MiniGame.gI().MiniGame_S1_Gem;
        long defaultBet = ConSoMayManGem.DEFAULT_BET;
        long defaultWin = game.calcTotalWin(defaultBet);
        createForm(pl, CON_SO_MAY_MAN_NGOC,
                "Chọn số từ 0 đến 99\nCược: " + defaultBet + " ngọc xanh | Thắng: " + defaultWin
                        + " ngọc (= giải " + game.rewardAmount + " + cược " + defaultBet + ")",
                new SubInput("Số bạn chọn", NUMERIC));
    }

    public void createFormConSoMayManGoldTuyChon(Player pl) {
        ConSoMayManGold game = MiniGame.gI().MiniGame_S1_Gold;
        String timer = game.getTimerDisplayLine();
        createForm(pl, CON_SO_MAY_MAN_VANG_MUC,
                "Giải vòng này: " + game.rewardAmount + " thỏi vàng\n"
                        + "Thắng nhận = giải + mức cược (VD cược 10 → " + (game.rewardAmount + 10) + " thỏi)\n"
                        + timer,
                new SubInput("Số bạn chọn (0-99)", NUMERIC),
                new SubInput("Mức cược (thỏi) | Thắng=giải+mức", NUMERIC));
    }

    public void createFormConSoMayManGemTuyChon(Player pl) {
        ConSoMayManGem game = MiniGame.gI().MiniGame_S1_Gem;
        String timer = game.getTimerDisplayLine();
        createForm(pl, CON_SO_MAY_MAN_GEM_MUC,
                "Giải vòng này: " + game.rewardAmount + " ngọc xanh\n"
                        + "Thắng nhận = giải + mức cược (VD cược 50 → " + (game.rewardAmount + 50) + " ngọc)\n"
                        + timer,
                new SubInput("Số bạn chọn (0-99)", NUMERIC),
                new SubInput("Mức cược (ngọc) | Thắng=giải+mức", NUMERIC));
    }

    private void refreshConSoMayManGemMenu(Player player) {
        Npc npc = NpcManager.getNpc(ConstNpc.LY_TIEU_NUONG);
        if (npc instanceof LyTieuNuong) {
            ((LyTieuNuong) npc).refreshConSoMayManGemMenu(player);
        }
    }

    private void refreshConSoMayManGoldMenu(Player player) {
        Npc npc = NpcManager.getNpc(ConstNpc.LY_TIEU_NUONG);
        if (npc instanceof LyTieuNuong) {
            ((LyTieuNuong) npc).refreshConSoMayManGoldMenu(player);
        }
    }
